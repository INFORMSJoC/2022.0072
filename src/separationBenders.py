...
class MySeparationBenders:
    ...
    def initSeparationHelper(self, instance, attr):
        ...
        for idx in range(num_self):
            matrix.append(
                cplex.SparsePair(ind=[idx_4b[idx], idx_4e[idx]], val=[1.0, 1.0])
            )
            rhs.append(0.0)
            constraint_senses.append("G")
        ...
        model.linear_constraints.add(
            rhs=rhs, lin_expr=matrix, names=constraint_name, senses=constraint_senses
        )
        return model
    ...
    def separate(self, supports, instance, zx, zy, epsilon=1e-5):
        ...
        if modelX_status == 1 and modelY_status == 1:
            self.constructLazyConstraint(
                modelX, modelY, supports, instance, zx, zy, epsilon
            )
    ...
    def constructLazyConstraint(
        self, modelX, modelY, supports, instance, zx, zy, epsilon
    ):
        supportX, supportY, supportX_M, supportY_M = supports
        modelX_status = modelX.solution.get_status()
        modelY_status = modelY.solution.get_status()
        ...
        if modelX_status == 1 and modelY_status == 1:
            ...
            if zx > modelX.solution.get_objective_value() + epsilon:
                ...
                values, rhs = self.lazyConstraintHelper(
                    instance, modelX.solution.get_values(), "x"
                )
                values.append(-1.0)
            ...
            if zy > modelY.solution.get_objective_value() + epsilon:
                ...
                values, rhs = self.lazyConstraintHelper(
                    instance, modelY.solution.get_values(), "y"
                )
                values.append(-1.0)
        ...
        if modelX_status == 2:  #
            indices = [idx for idx in range(instance._xNum + instance._yNum)]
            values, rhs = self.lazyConstraintHelper(
                instance, modelX.solution.advanced.get_ray(), "x"
            )
        ...
        if modelY_status == 2:
            indices = [idx for idx in range(instance._xNum + instance._yNum)]
            values, rhs = self.lazyConstraintHelper(
                instance, modelY.solution.advanced.get_ray(), "y"
            )
            self._lazy_constraint[1] = cplex.SparsePair(ind=indices, val=values)
            self._lazy_rhs[1] = rhs
