...
def solve_addPrimalX(supports, instance):
    ...
    sx_matrix.append(
        cplex.SparsePair(ind=sx_indices, val=[1.0 for i in range(len(sx_indices))])
    )
    sx_rhs.append(1.0)
    ...
    for curr in range(instance._xNum):
        ...
        sx_matrix.append(
            cplex.SparsePair(
                ind=sx_indices + [slackIndex, instance._yNum], val=values + [1.0, -1.0]
            )
        )
        sx_rhs.append(0.0)
        sx_constraint_senses.append("E")
    ...
    modelX.linear_constraints.add(
        rhs=sx_rhs,
        lin_expr=sx_matrix,
        names=sx_constraint_name,
        senses=sx_constraint_senses,
    )
...
def solve_addPrimalY(supports, instance):
    ...
    sy_matrix.append(
        cplex.SparsePair(ind=sy_indices, val=[1.0 for i in range(len(sy_indices))])
    )
    sy_rhs.append(1.0)
    ...
    for curr in range(instance._yNum):
        ...
        sy_matrix.append(
            cplex.SparsePair(
                ind=sy_indices + [slackIndex, instance._xNum], val=values + [1.0, -1.0]
            )
        )
        sy_rhs.append(0.0)
        sy_constraint_senses.append("E")
    ...
    modelY.linear_constraints.add(
        rhs=sy_rhs,
        lin_expr=sy_matrix,
        names=sy_constraint_name,
        senses=sy_constraint_senses,
    )
