...
class GameTheoryObj(object):
    ...
    def generate(self, x, y):
        ...
        for i in range(xNum):
            ...
            for j in range(yNum):
                ...
                self._XRange = [
                    min(self._XRange[0], aDict[i][j]["x"]),
                    max(self._XRange[1], aDict[i][j]["x"]),
                ]
                self._YRange = [
                    min(self._YRange[0], aDict[i][j]["y"]),
                    max(self._YRange[1], aDict[i][j]["y"]),
                ]
        #
        return aDict
    ...
    def getMax(self):
        return (
            float(np.amax(self._X)),
            float(np.amax(self._Y)),
            float(np.amax(self._X + self._Y)),
        )
        # return 400, 500, 900
