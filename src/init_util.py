...
def quickSolver(supports, instance, nonDominated):
    ...
    py1 = (utility[i2][j2]["x"] - utility[i1][j2]["x"]) / (
        utility[i2][j2]["x"]
        - utility[i1][j2]["x"]
        + utility[i1][j1]["x"]
        - utility[i2][j1]["x"]
    ...
    py2 = 1.0 - py1
    px1 = (utility[i2][j2]["y"] - utility[i2][j1]["y"]) / (
        utility[i2][j2]["y"]
        - utility[i1][j2]["y"]
        + utility[i1][j1]["y"]
        - utility[i2][j1]["y"]
...
def initMaster(masterP, instance):
    ...
    c_variableName = (
        ["x%d" % i for i in range(instance._xNum)]
        + ["y%d" % j for j in range(instance._yNum)]
        + ["zx", "zy"]
    )
    ...
    c_constraints.append(
        cplex.SparsePair(
            ind=[i for i in range(instance._xNum)],
            val=[1.0 for i in range(instance._xNum)],
        )
    )
    c_constraints.append(
        cplex.SparsePair(
            ind=[j + instance._xNum for j in range(instance._yNum)],
            val=[1.0 for j in range(instance._yNum)],
        )
    )
    ...
    masterP.variables.add(
        names=c_variableName, lb=c_lb, ub=c_ub, types=c_type, obj=c_objective
    )
    masterP.objective.set_sense(masterP.objective.sense.maximize)
    masterP.linear_constraints.add(
        lin_expr=c_constraints, rhs=c_rhs, names=c_names, senses=c_senses
    )
    ...
    if instanceMaxIdx:
        masterP.MIP_starts.add(
            cplex.SparsePair(
                ind=[
                    int(instanceMaxIdx[0]),
                    int(instanceMaxIdx[1] + instance._xNum),
                    int(zx_idx),
                    int(zy_idx),
                ],
                val=[1.0, 1.0, instanceXMax, instanceYMax],
            ),
            masterP.MIP_starts.effort_level.auto,
        )
        ...
        if maxSupport:
            masterP.MIP_starts.add(
                cplex.SparsePair(
                    ind=maxSupport + [int(zx_idx), int(zy_idx)], val=[1.0] * 4 + maxObj
                ),
                masterP.MIP_starts.effort_level.auto,
            )
            warmStartSolution = [0.0 for i in range(len(c_lb))]
...
def initMaster_Benders(masterP, instance, warmStartSolution):
    ...
    if warmStartSolution:
        ...
        masterP.MIP_starts.add(
            cplex.SparsePair(ind=indices, val=warmStartSolution),
            masterP.MIP_starts.effort_level.no_check,
        )
    return masterP
