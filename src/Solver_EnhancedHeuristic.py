        ...
        if add_heuristic:
            self.set_solution(
                cplex.SparsePair(
                    ind=[
                        i
                        for i in range(self._instance._xNum + self._instance._yNum + 2)
                    ],
                    val=data,
                ),
        ...
        if add_corresponding:
            self.set_solution(
                cplex.SparsePair(
                    ...
                    val=correspondingAction,
                ),
                objective_value=correspondingAction[-1] + correspondingAction[-2],
            )
            correspondingAction = []
            add_corresponding = False
...
class MyLazyConstraintCallBack(LazyConstraintCallback):
        ...
        try:
            ...
            if feas_52 == True:
                ...
                if not resultX and resultY:
                    self.add_local(
                        constraint=mySeparation._lazy_constraint[0],
                        sense="G",
                        rhs=mySeparation._lazy_rhs[0],
                    )
                    usercutNum += 1
                if not resultY and resultX:
                    self.add_local(
                        constraint=mySeparation._lazy_constraint[1],
                        sense="G",
                        rhs=mySeparation._lazy_rhs[1],
                    )
                    usercutNum += 1
                ...
                if resultX and resultY:
                    ...
                    resultX, objX = solve_addPrimalX(supports, self._instance)
                    resultY, objY = solve_addPrimalY(supports, self._instance)
                    self._feas51X, self._feas51Y = self._getMax5.getV_51(
                        self.get_upper_bounds(), self.get_lower_bounds()
                    )
    ...
    def add_optimality_cut(self, objX, objY):
        ...
        ySupport = np.array(
            data[self._instance._xNum : self._instance._xNum + self._instance._yNum]
        )
        ...
        if pre_val < self.get_incumbent_objective_value():
            ...
            if v1 < -10 or v2 < -10:
                ...
                if status and v >= 0:
                    ...
                    z_constraint = cplex.SparsePair(
                        ind=[zx_idx, zy_idx], val=[1.0, 1.0]
                    )
                    self.add_local(constraint=z_constraint, sense="L", rhs=v)
                    usercutNum += 1
                    ...
                    if self.get_incumbent_objective_value() + epsilon <= v:
                        ...
                        correspondingAction = self._instance.getCorrespondingAction(
                            solution
                        )
                        ...
                        if sum_x == sum_y:
                            ...
                            resultX, objX = solve_addPrimalX(supports, self._instance)
                            if resultX:
                                resultY, objY = solve_addPrimalY(
                                    supports, self._instance
                                )
                                if resultY:
                                    if (
                                        objX + objY
...
def script(instance, masterOut, timelimit, threadNum):
    ...
    masterPEnhance, preprocessTime, processType, warmStartSolution = initMaster(
        masterPEnhance, instance
    )

    masterPEnhance.parameters.preprocessing.presolve.set(
        masterPEnhance.parameters.preprocessing.presolve.values.off
    )
    ...
    if processType:
        branchCallbackEnhance = masterPEnhance.register_callback(
            MyBranchCallBackEnhance
        )
    else:
        branchCallbackEnhance = masterPEnhance.register_callback(
            MyBranchCallBackEnhance2
        )
    ...
    masterPEnhance.parameters.mip.strategy.search.set(
        masterPEnhance.parameters.mip.strategy.search.values.traditional
    )
    ...
    if (
        masterPEnhance_status == 101
        or masterPEnhance_status == 102
        or masterPEnhance_status == 107
    ):
        ...
        with open(masterOut, "a") as fout:
            ...
            csvWriter.writerow(
                [
                    instance._xNum,
                    instance._yNum,
                    masterPEnhance_status,
                    masterPEnhance.solution.get_objective_value(),
                    masterPEnhance_time,
                    masterPEnhance.solution.progress.get_num_nodes_processed(),
                    masterPEnhance.solution.MIP.get_mip_relative_gap(),
                    supportsize,
                    usercutNum,
                    preprocessTime,
                ]
...
def wrapper():
    ...
    parser.add_argument(
        "--xAction", type=int, default=6, help="number of actions for row player"
    )
    parser.add_argument(
        "--yAction", type=int, default=6, help="number of actions for column player"
    )
    parser.add_argument(
        "--xMin", type=float, default=100.0, help="min utility for row player"
    )
    parser.add_argument(
        "--yMin", type=float, default=100.0, help="min utility for column player"
    )
    parser.add_argument(
        "--xMax", type=float, default=200.0, help="max utility for row player"
    )
    parser.add_argument(
        "--yMax", type=float, default=200.0, help="max utility for row player"
    )
    parser.add_argument("--create", type=int, default=0, help="if create new instances")
    parser.add_argument(
        "--total", type=int, default=1, help="total new instance creating"
    )
    parser.add_argument(
        "--masterResult",
        type=str,
        default="resultMaster6.csv",
        help="result file for master problem",
    )
    parser.add_argument("--timelimit", type=float, default=100.0, help="time limit")
    parser.add_argument(
        "--threadsNum", type=int, default=1, help="threads number parameter"
    )

    args = parser.parse_args()
    masterOut = "../results/%s" % args.masterResult
