...
def script(
    instance, lemkeOut, timelimit, threadNum
):  # add sum of support for X equals Y --> may need to deactivate
    ...
    with open(lemkeOut, "a") as fout:
        ...
        csvWriter.writerow(
            [instance._xNum, instance._yNum, lemke_time, str(result)[:100]]
        )
        fout.flush()
...
def wrapper():
    ...
    lemkeOut = "../results/%s" % args.lemkeOut
