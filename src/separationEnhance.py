...
class MyBranchCallBackEnhance(
    BranchCallback
):  # get the id to check if it's still in the root node
    def __call__(self):
        if self.get_node_ID() == 0:
            ...
            self.make_branch(
                ...
                constraints=[
                    (
                        cplex.SparsePair(
                            ind=[i for i in range(varNum)],
                            val=[1.0 for i in range(varNum)],
                        ),
                        "E",
                        4.0,
                    )
...
class MyBranchCallBackEnhance2(
    BranchCallback
):  # get the id to check if it's still in the root node
    def __call__(self):
        if self.get_node_ID() == 0:
...
class MySeparationEnhance:
    ...
    def initSeparationEnhance(self, instance):
        ...
        self.sx_matrix.append(
            cplex.SparsePair(
                ind=sx_indices[:-1],
                val=[-1.0 for i in range(instance._xNum)]
                + [-1.0 for i in range(instance._xNum)],
            )
        )
        self.sx_rhs.append(0.0)
        ...
        modelX.variables.add(
            names=self.sx_variableName,
            lb=self.sx_lb,
            ub=self.sx_ub,
            obj=self.sx_objective,
        )
        modelX.linear_constraints.add(
            rhs=self.sx_rhs,
            lin_expr=self.sx_matrix,
            names=self.sx_constraint_name,
            senses=self.sx_constraint_senses,
        )
        ...
        self.sy_matrix.append(
            cplex.SparsePair(
                ind=sy_indices[:-1],
                val=[-1.0 for i in range(instance._yNum)]
                + [-1.0 for i in range(instance._yNum)],
            )
        )
        self.sy_rhs.append(0.0)
        ...
        modelY.variables.add(
            names=self.sy_variableName,
            lb=self.sy_lb,
            ub=self.sy_ub,
            obj=self.sy_objective,
        )
        modelY.linear_constraints.add(
            rhs=self.sy_rhs,
            lin_expr=self.sy_matrix,
            names=self.sy_constraint_name,
            senses=self.sy_constraint_senses,
        )
