...
def script(instance, bendersOut, timelimit, threadNum):
    ...
    masterBenders.parameters.preprocessing.presolve.set(
        masterBenders.parameters.preprocessing.presolve.values.off
    )
    ...
    lazyConstraintBenders = masterBenders.register_callback(
        MyLazyConstraintBendersCallBack
    )
    lazyConstraintBenders._instance = instance
    lazyConstraintBenders._separation = separationBenders

    masterBenders.parameters.mip.strategy.search.set(
        masterBenders.parameters.mip.strategy.search.values.traditional
    )
    ...
    if (
        masterBenders_status == 101
        or masterBenders_status == 102
        or masterBenders_status == 107
    ):
        ...
        with open(bendersOut, "a") as fout:
            ...
            csvWriter.writerow(
                [
                    ...
                    np.sum(
                        [
                            masterBenders.solution.MIP.get_num_cuts(cutType)
                            for cutType in [
                                0,
                                1,
                                2,
...
def wrapper():
    ...
    parser.add_argument(
        "--bendersResult",
        type=str,
        default="resultBenders6.csv",
        help="result file for Benders problem",
    )
    ...
    BendersOut = "../results/%s" % args.bendersResult
